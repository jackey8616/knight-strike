...
│   │   ├── overflow.ts       # 歷史遺留 no-op stub（未被 tick 呼叫）
...
### 9.1 既有 scaffolding 與 Three.js（已清理）

早期 scaffolding（`src/game` / `map` / `unit` / `manager` / `event` / `namedMap`）與 Three.js（`three` / `@types/three`）已於重製過程移除；render / input / ui 全面改用 Pixi.js v8。相關歷史見 git log。

### 9.2 `src/engine/overflow.ts` 為 no-op orphan

`applyCastleOverflow` 直接 `return state`，未被 `tick.ts` 呼叫。castle overflow 機制隨舊 AI 設計下線；保留 export 簽名，未來不需要可刪。

### 9.3 Node 版本與 CI

- `.nvmrc` / `engines.node` 鎖 Node 22；`pnpm build`（Vite 8）在 Node 18 會 fail。`typecheck` / `test` / `lint` 在 Node 18 仍可跑，但發布前務必 `nvm use` 切 22。
- `.github/workflows/ci.yml` 在 PR 跑 typecheck / lint / test / build；`deploy.yml`（GitHub Pages）依賴 CI 通過後才推。
