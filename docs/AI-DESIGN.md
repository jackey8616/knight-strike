...
>
> **關鍵發現（playtest）**：實作 AI 時發現——房屋只產 **100 人小隊**、若無「友軍合併」則經濟永遠養不出能破城（城堡 300 耐久）的大軍，4-AI 對局永遠 ongoing。故新增 **友軍同格合併**（PRD §4.7，修正 v2_spec §3.2「不能合併人數」之框架）：友軍堆疊成一支大軍，戰鬥仍部隊間 1v1。這是 AI「④ 集結」能成軍的前提。
...
| 進攻人數門檻 | 400 | 250 | 150 | 派軍進攻所需最低人數 |
| 稅率 | 0% | 15% | 25% | 高稅 = 多金養兵、慢成長 |
...
> **平衡現況（M12 提交時）**：加入友軍合併後 4-AI 對局**會收斂**（不再永遠 ongoing）；但對稱地圖 + 300 耐久城堡下目前多為 stalemate（到 tick 上限未分單一勝者）。**balance 為持續迭代項**（城堡耐久 / 進攻門檻 / 破壞公式 / 地圖皆 §6 flagged-tunable）——M12 交付「可運作、決定論、有測試」的 AI，balance 收斂為後續 tune（不阻擋 milestone）。
