...
> **PRD 同步基準：v2.0（Lord Monarch 經濟模型，全面取代 v1 原型）**。AC 編號用 PRD §9 的 `AC-01`..`AC-42`。**v1 原型 milestone（舊 M1–M4：count-tier engine / Pixi UI / playtest / deploy）已被取代**，封存於 git 標籤 `archive/prd-v2.5-prototype` 與 git log；下列 v2 milestone **延續 M5+ 編號**（branch 名 `feature/M5-...` 不與 v1 衝突）。
