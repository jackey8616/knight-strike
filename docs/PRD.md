...
**Version**: v1.6
**Status**: 鄰邊戰鬥 + 兩階段 claim + 拖曳征服行軍 + **地形（terrain：山/水不可通行、丘/林防禦減傷，seeded 程序生成）**
**Changelog**:
...
- **可選棋盤大小（v1.6）**：`N×N` 方格，玩家可選 **11 / 15 / 19 / 27**（預設 **19**；經 `?size=N` URL 參數記憶，切換 = 開新局）。主城恆置於四角（`(0,0) / (N-1,0) / (0,N-1) / (N-1,N-1)`），中立據點置於中央 `(⌊N/2⌋, ⌊N/2⌋)`。棋盤 fit-to-viewport 自動縮放，地形隨大小程序生成（§3.9）。早期 11x11 為原始尺寸。
...
- 任意 / 非方形棋盤、更多尺寸選項（目前 v1.6 已支援 11 / 15 / 19 / 27 方形，見 §3.1）
- 子城 / 多城制
- AI 難度分級
- 多人連線
- 音樂與音效
